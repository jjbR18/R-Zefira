...
Imported['EquipAndShopStatusR'] = 1.08;

/*:
 * @plugindesc ver1.08/装備&ショップシーンのステータス表示を改造します。
 * @author Yana
 *
 * @param 【ステータスの設定】
 *
...
(function(){
    ...
    var effectName = String(parameters['Effect Name'] || '有效度');
    var downName = String(parameters['Down Name'] || '下降');
    var itemTypeText = String(parameters['Item Type Text'] || '道具类型');
    var consumeText = String(parameters['Consume Text'] || '消耗');
    var damageTypeText = String(parameters['Damage Type Text'] || '伤害型');
    var priceText = String(parameters['Price Text'] || '价值');
    var scopeText = String(parameters['Scope Text'] || '效果范围');
    var occasionText = String(parameters['Occasion Text'] || '可以使用时间');
    var repeatText = String(parameters['Repeat Text'] || '发动次数');
    var tpGainText = String(parameters['TpGain Text'] || '获得ＴＰ');
    var turnText = String(parameters['Turn Text'] || '回合');
    var escapeText = String(parameters['Escape Text'] || '逃跑');
    var equipTypeText = String(parameters['Equip Type Text'] || '装备类型');
    var weaponTypeText = String(parameters['Weapon Type Text'] || '武器类型');
    var armorTypeText = String(parameters['Armor Type Text'] || '防具类型');
    var scopeVocab = String(parameters['Scope Vocab'] || '无,单体敌人,全体敌人,随机一名敌人,随机两名敌人,随机三名敌人,随机四名敌人,单体友方,全体友方,单体友方(濒死),全体友方(濒死),使用者').split(',');
    var occasionVocab = String(parameters['Occasion Vocab'] || '平时,战斗,菜单,无法使用').split(',');
    var damageVocab = String(parameters['Damage Vocab'] || '无,HP伤害,MP伤害,HP回复,MP回复,HP吸收,MP吸收').split(',');
    var itemTypeVocab = String(parameters['Item Type Vocab'] || '通常道具,贵重品,隐藏道具A,隐藏道具B').split(',');
    var consumeVocab = String(parameters['Consume Vocab'] || '消耗,不消耗').split(',');
    var xparamNames = String(parameters['Xparam Names'] || '命中,回避,暴击率,魔法回避,会心回避,魔法反射,反击,HP再生率,MP再生率,TP再生率').split(',');
    var sparamNames = String(parameters['Sparam Names'] || '被攻击率,防御效率,回复效率,药品知识,MP消耗率,TP填充率,物理伤害率,魔法伤害率,床伤害率,经验值获取率').split(',');
    var effectNames = String(parameters['Effects Names'] || 'HP回复,HP伤害,MP回复,MP伤害,TP增加,状态附加,状态解除,强化附加,弱化附加,强化解除,弱化解除,特殊效果,成长,学会技能,通常').split(',');
    ...
    Game_Actor.prototype.pictureName = function() {
        ...
        if (this._pictureName === undefined){
            ...
            if (this.actor().meta['状态图']){ this._pictureName = this.actor().meta['状态图'] }
        }
    ...
    Scene_Equip.prototype.smallSlotWindow = function() {
        ...
        this._slotWindow.height = this._slotWindow.contents.fontSize + this._slotWindow.standardPadding() * 2 + 8;
        var wy = this._slotWindow.y + this._slotWindow.height;
        var wh = Graphics.boxHeight - wy;
