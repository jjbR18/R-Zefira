...
// zarsuke add-start
//    }
    } else if (command === 'changeMenuLabel') {
      var idx = args[0];
      TMPlugin.MenuLabel.Labels[idx].id = args[1];
      TMPlugin.MenuLabel.Labels[idx].name = args[2];
    }
// zarsuke add-end
  };
...
// zarsuke del-end
// zarsuke add-start
      var label =  JSON.parse(JSON.stringify(TMPlugin.MenuLabel.Labels[i]));
      label.id = $gameVariables.value(i + 205);
      if (0 != label.id) n++;
// zarsuke add-start
    }
    return n;
...
// zarsuke add-start
      var label =  JSON.parse(JSON.stringify(TMPlugin.MenuLabel.Labels[i]));
      label.id = $gameVariables.value(i + 205);
      label.name = $gameVariables.value(i + 201);
      var value = $gameVariables.value(label.id);
      if (0 != label.id) {
// zarsuke add-end
