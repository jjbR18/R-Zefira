...
//                augmentType: augmentType, dataType: dataType});
    ...
    DreamX.RandomPrefixSuffix.makeItem = function (baseItem) {
        ...
        // zarsuke:add-start
        $gameVariables.setValue(200, newItem.name);
        // notedataから装備品のテキストカラー取得
        $gameVariables.setValue(199, 0);
        var notedata = newItem.note.split(/[\r\n]+/);
        for (var i = 0; i < notedata.length; i++) {
          var line = notedata[i];
          if (line.match(/<(?:TEXT COLOR):[ ](\d+)>/i)) {
            $gameVariables.setValue(199, parseInt(RegExp.$1));
          }
        }
        // ここまで
        // zarsuke:add-end
    ...
    Game_Party.prototype.gainItem = function (item, amount, includeEquip) {
        ...
        // zarsuke:add-start
            $gameVariables.setValue(200, item.name);
            // notedataから装備品のテキストカラー取得
            $gameVariables.setValue(199, 0);
            var notedata = item.note.split(/[\r\n]+/);
            for (var i = 0; i < notedata.length; i++) {
