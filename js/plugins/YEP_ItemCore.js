...
// Zarsuke Add-start
    // 売却したアイテムに付属している+の値を取得する
    var ParamBonus = Number(0);
    index = this._item.name.indexOf(" +");
    if (index != -1)
    {
        ParamBonus = parseInt(this._item.name.slice(index));
        if (this._item.wtypeId)
        {
            ParamBonus += $gameVariables.value(141);
            $gameVariables.setValue(141, ParamBonus);
        }
        else if (this._item.atypeId)
        {
            ParamBonus += $gameVariables.value(142);
            $gameVariables.setValue(142, ParamBonus);        
        }
    }
// Zarsuke Add-end
