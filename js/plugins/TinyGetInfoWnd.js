...
// zarsuke:add-start
  Window_GetInfo.prototype.drawItemName = function(item, x, y, width) {
      width = width || 312;
      if (item) {
          var iconBoxWidth = Window_Base._iconWidth + 4;
          this.resetTextColor();
          // notedataから装備品のテキストカラー取得
          this.changeTextColor(this.textColor($gameVariables.value(199)));
          // ここまで
...
// zarsuke:add-end
  ...
  Window_GetInfo.prototype.drawContents = function(id, type, text, value) {
    ...
    var data_base = this.determineItem(type, id);
    var data = JSON.parse(JSON.stringify(data_base));
    data.name = $gameVariables.value(200);
    // fill background
